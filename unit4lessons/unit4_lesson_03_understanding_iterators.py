...
def test_multiple_iterators():
    ...
    assert __ == iter1.__next__()
    assert __ == iter2.__next__()

    assert __ == list(iter1)
    assert __ == list(iter2)
...
def test_iter_on_iterator():
    ...
    assert ___ == convert_to_list(iter1)
...
def test_for():
    ...
    for x in iter1:
        result.append(x)
    assert ___ == result  #what happened here?
